...
const FALLBACK: Record<'en' | 'es' | 'id' | 'ru', string> = { en: 'en', es: 'es', id: 'id', ru: 'ru' }
function pick(map: Record<string, Copy>, locale: string): Copy {
  return map[locale] ?? map.en
}

const SEO_PAGES: { slug: string; order: number; copy: Record<string, Copy> }[] = [
  {
    slug: 'home',
    order: 1,
    copy: {
...
const FAQ_ITEMS: Record<string, { question: string; answer: string }[]> = {
  en: [
    { question: 'How much does a private driver cost in Bali?', answer: 'A private car with Made starts around $65 for a half-day route and goes up to $160 for the 2–4 day Grand Bali Loop. Every route includes the car, petrol, an English-speaking guide, bottled water and hotel pick-up in the Ubud area.' },
  ...
  es: [
    { question: '¿Cuánto cuesta un conductor privado en Bali?', answer: 'Un coche privado con Made cuesta desde unos $65 para una ruta de medio día hasta $160 para el Grand Bali Loop de 2 a 4 días. Cada ruta incluye el coche, gasolina, guía en inglés, agua embotellada y recogida en el hotel en la zona de Ubud.' },
  ...
  id: [
    { question: 'Berapa biaya sopir pribadi di Bali?', answer: 'Mobil pribadi dengan Made mulai sekitar $65 untuk rute setengah hari hingga $160 untuk Grand Bali Loop 2–4 hari. Setiap rute termasuk mobil, bensin, pemandu berbahasa Inggris, air mineral, dan penjemputan hotel di area Ubud.' },
  ...
  ru: [
    { question: 'Сколько стоит частный водитель на Бали?', answer: 'Частная машина с Мейдом стоит от $65 за маршрут на полдня до $160 за Grand Bali Loop на 2–4 дня. Каждый маршрут включает автомобиль, бензин, англоговорящего гида, воду и трансфер из отеля в районе Убуда.' },
